...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { autoload: @option.autoload, name: @option.name, value: @option.value } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { autoload: @option.autoload, name: @option.name, value: @option.value } }
